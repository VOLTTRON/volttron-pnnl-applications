...
from .restful_simulation_integration import EnergyPlusSimIntegration
...
class EnergyPlusAgent(Agent):
    ...
    def subscribe(self):
        ...
        :return:
        """
        for name, obj in self.EnergyPlus_sim.inputs.items():
    ...
    def publish_all_outputs(self):
        ...
        :return:
        """
        _now = _now = self.EnergyPlus_sim.current_sim_time
        _log.info(f"Publish the building response for timestamp: {type(_now)} - {_now}.")

        headers = {headers_mod.DATE: utils.format_timestamp(_now), headers_mod.TIMESTAMP: utils.format_timestamp(_now)}
        topics = collections.OrderedDict()

        for name, obj in self.outputs.items():
        ...
        for topic, obj in topics.items():
            if obj['values'] is not None:
                for value in obj['values']:
                    ...
                    self.vip.pubsub.publish('pubsub', topic, headers, out)
            if obj['fields'] is not None:
                out = obj['fields']
                #_log.info(f"Sending DATA: {topic} {out}")
                while True:
                    try:
                        self.vip.pubsub.publish('pubsub', topic, headers, out)
                    except Exception as e:
                        _log.debug("Again ERROR: retrying publish {}".format(e))
                        gevent.sleep(0.1)
                        continue
                    break
            self.num_of_pub += 1
    ...
    def find_best_match(self, topic, func):
        """
        Find all input objects that best match the topic
        :param topic: topic to  match
        :return:
    ...
    def do_work(self):
        ...
        if self.EnergyPlus_sim.real_time_periodic and self.rt_periodic is None:
            _log.debug("EPLUS setup periodic!")
    ...
    def check_advance(self):
        ...
        if not self.EnergyPlus_sim.real_time_flag:
            ...
            if self.EnergyPlus_sim.co_sim_timestep is not None and self.cosim_sync_counter < self.EnergyPlus_sim.co_sim_timestep:
    ...
    def get_point(self, topic, **kwargs):
        ...
        """
        obj = self.find_best_match(topic, self.get_inputs_from_topic)
        if obj is None or obj.get('value') is None:
            obj = self.find_best_match(topic, self.get_outputs_from_topic)
    ...
    def revert_device(self, requester_id, device_name, **kwargs):
        ...
        if objs is not None:
            ...
            _log.warning("Unable to revert " + device_name + ". No default defined!")
